...
export default class Database {
  ...
  fullLookup (account) {
    ...
    let sqlstmt = this.sql.prepare(
      'UPDATE Accounts SET inBank = Coalesce(Accounts.baseAmount+(SELECT SUM(amount) FROM OPERATION WHERE account_name =:name AND state like \'%fa-check-circle\' AND date<=:date), Accounts.baseAmount) WHERE name =:name2'
    )
    sqlstmt.run({
      ':name': account,
  ...
  editRecurringOperation (id, account, data, df) {
    // [amount, type, benef, cat, label, date, offset, timespan, time]
    if (account === null || account === undefined) throw new Error('No account provided', 'Database.class.js', 185)
    if (id === null || id === undefined) throw new Error('No id provided', 'Database.class.js', 186)
  ...
  editOperation (id, data, df) {
    if (data[0] === null || data[0] === undefined) throw new Error('No account provided', 'Database.class.js', 204)
  ...
  updateTable (account, date, state, amount) {
    ...
    // sqlstmt.sqlBinding = sqlstmt.bind // 'bind' already exist in Vue API
    sqlstmt.bind({
      ':account': account,
      ':date': date,
      ':state': state,
